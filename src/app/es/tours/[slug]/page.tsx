...
// ─────────────────────────────────────────────
// STATIC SPANISH TOUR PAGES
// ─────────────────────────────────────────────

export function generateStaticParams() {
  return toursEs.map((tour) => ({
    slug: tour.slug,
  }));
}
...
export async function generateMetadata({
  ...
  const tour = toursEs.find((item) => item.slug === slug);

  if (!tour) {
    return {
      title: {
        absolute: "Tour no encontrado | Moroccan Trip",
      },
  ...
  // Find the matching English tour using the shared ID.
  const englishTour = tours.find(
    (item) => item.id === tour.id
  );

  const spanishUrl =
    `https://moroccan-trip.com/es/tours/${tour.slug}`;

  const englishUrl = englishTour
    ? `https://moroccan-trip.com/tours/${englishTour.slug}`
    : null;
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `https://moroccan-trip.com${tour.image}`,
          alt: tour.title,
        },
      ],
    },
...
export default async function SpanishTourPage({
  params,
}: TourPageProps) {
  const { slug } = await params;

  const tour = toursEs.find(
    (item) => item.slug === slug
  );
  ...
  const touristTripSchema = {
    ...
    touristType: "Tour privado por Marruecos",
    ...
    itinerary: {
      ...
      itemListElement: tour.itinerary.map(
        (day, index) => ({
          "@type": "ListItem",
    ...
    // Schema.org duration uses ISO 8601.
    duration:
      `P${tour.booking.minimumDays}D`,
  ...
  // ─────────────────────────────────────────────
  // RELATED SPANISH TOURS
  // ─────────────────────────────────────────────

  const relatedTours = toursEs
    .filter(
      (item) => item.id !== tour.id
    )
    .map((item) => {
    ...
    .sort((a, b) => {
      ...
      return a.tour.title.localeCompare(
        b.tour.title,
        "es"
      );
    })
          ...
          <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[minmax(0,1fr)_330px] lg:px-10 lg:py-16">
            ...
            <div className="min-w-0 space-y-10">
              ...
              <TourSection id="overview">
                ...
                <SectionHeading
                  icon={<DocumentIcon />}
                  eyebrow="Descripción del tour"
                  title="Tu viaje por Marruecos"
                />
              ...
              <TourSection id="itinerary">
                ...
                <p className="mt-5 max-w-3xl text-[15px] leading-7 text-[hsl(var(--heading)/0.72)]">
                  Abre cada día para descubrir
                  la ruta, las paradas y las
                  experiencias previstas durante
                  tu viaje privado por Marruecos.
                </p>
              ...
              <TourSection id="route">
                ...
                <SectionHeading
                  icon={<MapPinIcon />}
                  eyebrow="Ruta del tour"
                  title="Sigue el recorrido por Marruecos"
                />

                <p className="mt-5 max-w-3xl text-[15px] leading-7 text-[hsl(var(--heading)/0.72)]">
                  Descubre las principales
                  paradas de tu tour privado y
                  consulta el recorrido en el
                  mapa interactivo.
                </p>
              ...
              <TourSection id="highlights">
                ...
                <SectionHeading
                  icon={<StarIcon />}
                  eyebrow="Lo mejor del tour"
                  title="Experiencias destacadas del viaje"
                />
              ...
              <TourSection id="accommodation">
                ...
                <SectionHeading
                  ...
                  title="Alojamiento durante el tour"
                />

                <p className="mt-5 max-w-3xl text-[15px] leading-7 text-[hsl(var(--heading)/0.72)]">
                  El alojamiento puede
                  adaptarse a tu nivel de
                  comodidad y estilo de viaje.
                </p>
              ...
              {tour.gallery.length >
                ...
                <TourSection id="gallery">
                  ...
                  <SectionHeading
                    icon={<GalleryIcon />}
                    eyebrow="Galería del tour"
                    title="Una mirada al viaje"
                  />
              ...
              {tour.faqs.length > 0 && (
                <TourSection id="faq">
                  ...
                  <p className="mt-5 max-w-3xl text-[15px] leading-7 text-[hsl(var(--heading)/0.72)]">
                    Encuentra respuestas a las
                    preguntas más habituales
                    sobre el itinerario,
                    alojamiento y tu experiencia
                    de viaje.
                  </p>
              ...
              {relatedTours.length > 0 && (
                <section className="rounded-[30px] border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-6 shadow-[0_2px_15px_rgba(0,0,0,0.025)] sm:p-8 lg:p-9">
                  ...
                  <p className="mt-5 max-w-2xl text-[15px] leading-7 text-[hsl(var(--text-secondary))]">
                    Descubre otros tours privados
                    por Marruecos con rutas,
                    ...
                    {relatedTours.map(
                      (
                        ...
                        <Link
                          ...
                          href={`/es/tours/${relatedTour.slug}`}
                        ...
                        >
                          ...
                          <div className="relative aspect-[4/3] overflow-hidden bg-[hsl(var(--surface-soft))]">
                            ...
                            <Image
                              ...
                              alt={`${relatedTour.title} en Marruecos`}
                              fill
                  ...
                  {tour.booking.customizable && (
                    <div className="mt-5 flex items-start gap-2.5 rounded-2xl bg-[hsl(var(--surface-soft))] p-4">
                      ...
                      <p className="text-xs leading-5 text-[hsl(var(--text-muted))]">
                        Este tour privado puede
                        personalizarse según tus
                        fechas, preferencias y
                        estilo de viaje.
                      </p>
